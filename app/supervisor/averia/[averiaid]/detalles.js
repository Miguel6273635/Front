...
export default function DetallesAveriaSupervisor() {
  ...
  const [causas, setCausas] = useState([]); // [{GrupoCodigo,Codigo,Descripcion}]
  const [searchCausa, setSearchCausa] = useState('');
  const [selectedCausa, setSelectedCausa] = useState(null); // {GrupoCodigo,Codigo,Descripcion}

  const fetchDetalle = async () => {
    try {
      setLoading(true);

      // ✅ Header (con token vía api.js)
      const resHdr = await api.get(
        `/api/odata/ZCS_GET_NOTIFICATION_SRV/NotificationHeaderSet('${encodeURIComponent(
          averiaid
        )}')`,
        { params: { $format: 'json' } }
      );
      ...
      // ✅ Normaliza CodeGroup/Coding por si vienen con otras llaves
      const codeGroup =
      ...
      // ✅ si ya trae CodeGroup/Coding, reflejar selección
      if (codeGroup && coding) {
        setSelectedCausa({
          GrupoCodigo: codeGroup,
          Codigo: coding,
          Descripcion: '',
        });
      }
  ...
  // (tu fallback Launchpad lo dejo, pero primero intentamos por tu API)
  ...
  const fetchCatalogoCausas = async () => {
    try {
      ...
      // ✅ 1) Primero tu API BTP (ya existe en tu backend)
      try {
      ...
      // ✅ 2) Fallback directo a Launchpad (puede requerir SSO)
  ...
  // ✅ AQUI ES LO IMPORTANTE: manda a /api/odata/ZCS_CHANGE_AVISO_SRV/NotificationHeaderSet
  const asignarCausa = async () => {
    ...
    // ✅ Usa NotifNo real si viene, si no usa averiaid
    ...
    try {
      ...
      const payload = {
        Orderid: notifNoToUse, // <-- así lo estás mandando tú
        NotifHeader: {
      ...
      const { data } = await api.post(
        ...
        payload,
        {
          // opcional: tu proxy ya lo pone si falta
          params: { "sap-client": "400", "sap-language": "ES" },
        }
      );

      // ✅ Algunos servicios devuelven {d:{...}} o directo.
      // Si quieres validar "Return", aquí puedes inspeccionarlo, pero mínimo:
      ...
      Alert.alert('Error', 'No se pudo asignar la causa en SAP.');
    } finally {
  ...
  // ✅ para bloquear/boton crear orden
          ...
          <View style={styles.modalCard}>
            ...
            {loadingCausas ? (
              <View style={{ paddingVertical: 18, alignItems: 'center' }}>
                ...
                <Text style={{ marginTop: 8, color: COLORS.text, fontSize: 12 }}>
                  Cargando catálogo…
                </Text>
              </View>
            ) : (
