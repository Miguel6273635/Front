  ...
  const fetchLista = useCallback(async () => {
    try {
      if (!correo) {
        setRows([]);
        setLoading(false);
        Alert.alert("Sin correo", "No se detectó el correo del técnico loggeado.");
        return;
      }
      ...
      console.error("Error No mantenimiento (técnico):", e?.response?.data || e?.message);
      setRows([]);
...
const styles = StyleSheet.create({
  ...
  pill: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 999, backgroundColor: "#FFF3CD" },
