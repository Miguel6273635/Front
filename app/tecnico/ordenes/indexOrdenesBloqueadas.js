...
// ===== Comparación por día en UTC =====
const isSameUtcDay = (a, b) => {
  if (!a || !b) return false;
  return getUtcYmd(a) === getUtcYmd(b);
};
...
export default function ListaOrdenesTecnico() {
  ...
  // También cuando cambian filtros
  useEffect(() => {
    fetchOrdenes();
  ...
  const renderItem = ({ item }) => {
    const startDateObj = parseSapDate(item?.start_date);
    const finishDateObj = parseSapDate(item?.finish_date);
    ...
    // Detectar No mantenimiento
    const estatusLower = String(item.estatus || '').toLowerCase();
    const isNoMantto =
      estatusLower.includes('no mant') || estatusLower.includes('no_mant');

    // ¿Es orden del día (hoy) en UTC?
    const today = new Date();
    const isTodayOrder = !!startDateObj && isSameUtcDay(startDateObj, today);

    // Solo permitir check-in si es de HOY
    const canDoCheckin = !isNoMantto && !hasCheckin && isTodayOrder;

    // Si no es de hoy y todavía no tiene check-in → se ve “deshabilitada”
    const isDisabledByDate = !isNoMantto && !hasCheckin && !isTodayOrder;

    // Color de estatus (incluye gris para No mantenimiento)
    const estatusColor =
      isNoMantto ? '#9E9E9E' :
    ...
    return (
      <TouchableOpacity
        ...
        style={[
          styles.card,
          { borderLeftWidth: 4, borderLeftColor: estatusColor },
          isNoMantto && styles.cardNoMant,
          isDisabledByDate && styles.cardDisabled, // <-- NUEVO
        ]}
      ...
      >
        <View style={styles.cardContent}>
          ...
          <View style={{ flex: 1 }}>
            ...
            {/* Etiqueta rápida cuando no es del día */}
            {isDisabledByDate && (
              <Text style={styles.disabledHint}>
                Check-in solo disponible el día de la orden.
              </Text>
            )}
          </View>
        </View>
        ...
        <View style={{ flexDirection:'row', gap:10, justifyContent:'flex-end', marginTop:12 }}>
          {isNoMantto ? (
            ...
            // ---- No tiene check-in aún ----
            <TouchableOpacity
              style={[
                styles.boton,
                { backgroundColor: canDoCheckin ? FIORI.accent : '#C9D1DB' }, // <-- NUEVO
              ]}
              onPress={(e)=> canDoCheckin && abrirModalCheckin(e, item.Orderid)}
              disabled={!canDoCheckin} // <-- NUEVO
            >
              <Text style={styles.botonTexto}>
                {canDoCheckin ? 'Check-in' : 'Check-in (no disponible)'}
              </Text>
            </TouchableOpacity>
          ) : (
            // ---- Ya tiene check-in ----
...
const styles = StyleSheet.create({
  ...
  // ---- Menú de 3 puntos (no usado, por si lo necesitas luego) ----
  cardMenu: {
    position: 'absolute',
    top: 6,
    right: 6,
