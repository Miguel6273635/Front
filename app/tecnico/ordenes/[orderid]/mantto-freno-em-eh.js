...
const Readonly = ({ children }) => (
  <View style={styles.readonly}><Text style={styles.readonlyText}>{String(children ?? '—')}</Text></View>
);
...
function PhotoBlock({ title, value, onChange }) {
  const pick = async (momento) => {
    try {
      ...
      // No es obligatorio conceder cámara; si la niega, puede elegir de la galería
...
export default function ManttoFrenoEmEhForm() {
  ...
  // 1–9 (igual que versión anterior) —–– (estados omitidos aquí por brevedad en el comentario)
  ...
  const torquePromAntes = useMemo(() => avg([torque1Antes, torque2Antes, torque3Antes]), [torque1Antes, torque2Antes, torque3Antes]);
  const torquePromDesp = useMemo(() => avg([torque1Desp, torque2Desp, torque3Desp]), [torque1Desp, torque2Desp, torque3Desp]);
  ...
  // ====== FOTOS (segunda hoja) ======
  // Cada clave contiene { antes: [{uri, note}], despues: [{uri, note}] }
  ...
  const load = useCallback(async () => {
    try {
      ...
      const { data, status } = await api.get(`/mantenimiento-freno-em-eh/datos/${orderid}`, {
      ...
      // Si en el futuro GET también trae fotos, aquí podrías setPhotos(...) con lo recibido.
    } catch (e) {
  ...
  const onGuardar = async () => {
    try {
      ...
      // Implementación sugerida: POST /mantenimiento-freno-em-eh/fotos
      // Campos: orderid, seccion, momento (antes|despues), nota (opcional), file (multipart)
      const uploadOk = await maybeUploadAllPhotos(String(orderid), photos, token);

      Alert.alert(
        'Listo',
        uploadOk ? 'Formulario y fotos guardados' : 'Formulario guardado. (Las fotos no se subieron: configura el endpoint)',
  ...
  const buildPayload = () => ({
    ...
    // meta de fotos (conteo, útil si quieres reflejarlo en PDF)
    fotos_meta: Object.fromEntries(
      Object.entries(photos).map(([k,v]) => [k, { antes: v?.antes?.length || 0, despues: v?.despues?.length || 0 }])
    ),
  });

  async function maybeUploadAllPhotos(orderidStr, photosObj, token) {
    // Cambia esta bandera a 'true' cuando tengas el endpoint activo
    const HAS_ENDPOINT = true; // si no lo tienes aún, pon false para saltar subida
    if (!HAS_ENDPOINT) return false;
      ...
      for (const [seccion, val] of entries) {
        for (const momento of ['antes', 'despues']) {
          for (const item of (val?.[momento] || [])) {
            ...
            // tipo mime tentativa según extensión
            ...
            if (!res || res.status >= 400) {
              ...
              // no cortamos todo por una falla de una foto; seguimos
            }
          }
        }
      }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#F5F7FB' }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 180 }}>
        ...
        <Card>
          <Row2>
            <Col><Label>Cliente</Label><Readonly>{auto?.cliente}</Readonly></Col>
          ...
          <Row2>
            <Col><Label>Partner (rol / id)</Label><Readonly>{`${auto?.partner_rol ?? '—'} / ${auto?.partner_id ?? '—'}`}</Readonly></Col>
          </Row2>
        </Card>
        ...
        <Card>
          <Row2>
            ...
            <Col><Label>Horario</Label><Input value={horario} onChangeText={setHorario} placeholder="Ej. 08:00–12:00" /></Col>
          </Row2>
          <Row2>
            <Col><Label>Control</Label><Input value={control} onChangeText={setControl} placeholder="Control…" /></Col>
            <Col><Label>Tipo de MT</Label><Input value={tipoMt} onChangeText={setTipoMt} placeholder="Preventivo/Correctivo…" /></Col>
          </Row2>
          <Row2>
            <Col><Label>Velocidad nominal (m/min)</Label><Input keyboardType="numeric" value={velNominal} onChangeText={setVelNominal} placeholder="0.0" /></Col>
            <Col><Label>Capacidad (kg)</Label><Input keyboardType="numeric" value={capacidadKg} onChangeText={setCapacidadKg} placeholder="0" /></Col>
          </Row2>
        ...
        {/* ……… SECCIONES 1–9 (igual que la versión anterior) ……… */}
        {/* 1) Pines y levas de freno */}
        <Section>1) Pines y levas de freno</Section>
        <Card>
          <Label>1a) Kit con lubricación</Label>
          <Row2>
            <Col><Label>Óxido (antes)</Label><YN value={pinesKitLubOxidoAntes} onChange={setPinesKitLubOxidoAntes} /></Col>
            <Col><Label>Óxido (después)</Label><YN value={pinesKitLubOxidoDesp} onChange={setPinesKitLubOxidoDesp} /></Col>
          </Row2>
          <Row2>
            <Col><Label>Lubricación (antes)</Label><BM value={pinesKitLubLubAntes} onChange={setPinesKitLubLubAntes} /></Col>
            <Col><Label>Lubricación (después)</Label><BM value={pinesKitLubLubDesp} onChange={setPinesKitLubLubDesp} /></Col>
          </Row2>

          <Label style={{ marginTop: 10 }}>1b) Kit libre de lubricación</Label>
          <Row2>
            <Col><Label>Giro libre (antes)</Label><BM value={pinesKitLibreGiroAntes} onChange={setPinesKitLibreGiroAntes} /></Col>
            <Col><Label>Giro libre (después)</Label><BM value={pinesKitLibreGiroDesp} onChange={setPinesKitLibreGiroDesp} /></Col>
          </Row2>
          <Row2>
            <Col><Label>Cubierta (antes)</Label><YN value={pinesKitLibreCubiertaAntes} onChange={setPinesKitLibreCubiertaAntes} /></Col>
            <Col><Label>Cubierta (después)</Label><YN value={pinesKitLibreCubiertaDesp} onChange={setPinesKitLibreCubiertaDesp} /></Col>
          </Row2>

          <Row2>
            <Col><Label>Revisado</Label><YN value={pinesRevisado} onChange={setPinesRevisado} /></Col>
            <Col><Label>Revisión SMA</Label><OkNec value={pinesRevisionSMA} onChange={setPinesRevisionSMA} /></Col>
          </Row2>
        </Card>
        ...
        <Card>
          <Row2>
            <Col><Label>Estándar (REF 1)</Label><Input value={torqueEstandarRef} onChangeText={setTorqueEstandarRef} placeholder="Valor ref" /></Col>
          </Row2>
          <Label style={{ marginTop: 8 }}>Mediciones</Label>
          <Row2>
            <Col><Label>1. antes</Label><Input keyboardType="numeric" value={torque1Antes} onChangeText={setTorque1Antes} /></Col>
            <Col><Label>1. después</Label><Input keyboardType="numeric" value={torque1Desp} onChangeText={setTorque1Desp} /></Col>
          </Row2>
          <Row2>
            <Col><Label>2. antes</Label><Input keyboardType="numeric" value={torque2Antes} onChangeText={setTorque2Antes} /></Col>
            <Col><Label>2. después</Label><Input keyboardType="numeric" value={torque2Desp} onChangeText={setTorque2Desp} /></Col>
          </Row2>
          <Row2>
            <Col><Label>3. antes</Label><Input keyboardType="numeric" value={torque3Antes} onChangeText={setTorque3Antes} /></Col>
            <Col><Label>3. después</Label><Input keyboardType="numeric" value={torque3Desp} onChangeText={setTorque3Desp} /></Col>
          </Row2>
          <Row2>
            <Col><Label>Promedio (antes)</Label><Readonly>{torquePromAntes ?? '—'}</Readonly></Col>
            <Col><Label>Promedio (después)</Label><Readonly>{torquePromDesp ?? '—'}</Readonly></Col>
          </Row2>
          <Row2>
            <Col><Label>Estado (antes)</Label><BM value={torqueEstadoAntes} onChange={setTorqueEstadoAntes} /></Col>
            <Col><Label>Estado (después)</Label><BM value={torqueEstadoDesp} onChange={setTorqueEstadoDesp} /></Col>
          </Row2>
          <Row2>
            <Col><Label>Revisado</Label><YN value={torqueRevisado} onChange={setTorqueRevisado} /></Col>
            <Col><Label>Revisión SMA</Label><OkNec value={torqueRevisionSMA} onChange={setTorqueRevisionSMA} /></Col>
          </Row2>
        </Card>
        ...
        <Card>
          <Row2>
            <Col><Label>Izquierdo (antes) mm (% opcional)</Label><Input value={resorteIzqAntes} onChangeText={setResorteIzqAntes} placeholder="Ej. 20 (5%)" /></Col>
            <Col><Label>Izquierdo (después)</Label><Input value={resorteIzqDesp} onChangeText={setResorteIzqDesp} placeholder="Ej. 20 (5%)" /></Col>
          </Row2>
          <Row2>
            <Col><Label>Derecho (antes)</Label><Input value={resorteDerAntes} onChangeText={setResorteDerAntes} placeholder="Ej. 20 (5%)" /></Col>
            <Col><Label>Derecho (después)</Label><Input value={resorteDerDesp} onChangeText={setResorteDerDesp} placeholder="Ej. 20 (5%)" /></Col>
          </Row2>
          <Row2>
            <Col><Label>Estado (antes)</Label><BM value={resorteEstadoAntes} onChange={setResorteEstadoAntes} /></Col>
            <Col><Label>Estado (después)</Label><BM value={resorteEstadoDesp} onChange={setResorteEstadoDesp} /></Col>
          </Row2>
          <Row2>
            <Col><Label>Revisado</Label><YN value={resorteRevisado} onChange={setResorteRevisado} /></Col>
            <Col><Label>Revisión SMA</Label><OkNec value={resorteRevisionSMA} onChange={setResorteRevisionSMA} /></Col>
          </Row2>
        </Card>
        ...
        <Card>
          <Row2>
            <Col><Label>Estándar (REF 2)</Label><Input value={emboloRef} onChangeText={setEmboloRef} placeholder="Valor ref" /></Col>
          </Row2>
          <Row2>
            <Col><Label>Recorrido Izq. (antes) mm</Label><Input keyboardType="numeric" value={emboloIzqAntes} onChangeText={setEmboloIzqAntes} /></Col>
            <Col><Label>Recorrido Izq. (después) mm</Label><Input keyboardType="numeric" value={emboloIzqDesp} onChangeText={setEmboloIzqDesp} /></Col>
          </Row2>
          <Row2>
            <Col><Label>Recorrido Der. (antes) mm</Label><Input keyboardType="numeric" value={emboloDerAntes} onChangeText={setEmboloDerAntes} /></Col>
            <Col><Label>Recorrido Der. (después) mm</Label><Input keyboardType="numeric" value={emboloDerDesp} onChangeText={setEmboloDerDesp} /></Col>
          </Row2>

          <Row2>
            <Col><Label>Desgaste ¿tiene? (antes)</Label><YN value={emboloDesgTieneAntes} onChange={setEmboloDesgTieneAntes} /></Col>
            <Col><Label>Desgaste ¿tiene? (después)</Label><YN value={emboloDesgTieneDesp} onChange={setEmboloDesgTieneDesp} /></Col>
          </Row2>
          <Row2>
            <Col><Label>Óxido ¿tiene? (antes)</Label><YN value={emboloOxidoTieneAntes} onChange={setEmboloOxidoTieneAntes} /></Col>
            <Col><Label>Óxido ¿tiene? (después)</Label><YN value={emboloOxidoTieneDesp} onChange={setEmboloOxidoTieneDesp} /></Col>
          </Row2>
          <Row2>
            <Col><Label>Lubricación (antes)</Label><BM value={emboloLubAntes} onChange={setEmboloLubAntes} /></Col>
            <Col><Label>Lubricación (después)</Label><BM value={emboloLubDesp} onChange={setEmboloLubDesp} /></Col>
          </Row2>
          <Row2>
            <Col><Label>Espesor de arandela (antes) mm</Label><Input keyboardType="numeric" value={emboloEspesorAntes} onChangeText={setEmboloEspesorAntes} /></Col>
            <Col><Label>Espesor de arandela (después) mm</Label><Input keyboardType="numeric" value={emboloEspesorDesp} onChangeText={setEmboloEspesorDesp} /></Col>
          </Row2>
          <Row2>
            <Col><Label>Revisado</Label><YN value={emboloRevisado} onChange={setEmboloRevisado} /></Col>
            <Col><Label>Revisión SMA</Label><OkNec value={emboloRevisionSMA} onChange={setEmboloRevisionSMA} /></Col>
          </Row2>
        </Card>
        ...
        <Card>
          <Row2>
            <Col><Label>Estándar (REF 3)</Label><Input value={contactoRef} onChangeText={setContactoRef} placeholder="Valor ref" /></Col>
          </Row2>
          <Row2>
            <Col><Label>Separación Izq. (antes) mm</Label><Input keyboardType="numeric" value={contactoSepIzqAntes} onChangeText={setContactoSepIzqAntes} /></Col>
            <Col><Label>Separación Izq. (después) mm</Label><Input keyboardType="numeric" value={contactoSepIzqDesp} onChangeText={setContactoSepIzqDesp} /></Col>
          </Row2>
          <Row2>
            <Col><Label>Separación Der. (antes) mm</Label><Input keyboardType="numeric" value={contactoSepDerAntes} onChangeText={setContactoSepDerAntes} /></Col>
            <Col><Label>Separación Der. (después) mm</Label><Input keyboardType="numeric" value={contactoSepDerDesp} onChangeText={setContactoSepDerDesp} /></Col>
          </Row2>
          <Row2>
            <Col><Label>Punto de contacto / cable / tuercas (antes)</Label><BM value={contactoPuntoBienAntes} onChange={setContactoPuntoBienAntes} /></Col>
            <Col><Label>Punto de contacto / cable / tuercas (después)</Label><BM value={contactoPuntoBienDesp} onChange={setContactoPuntoBienDesp} /></Col>
          </Row2>
          <Row2>
            <Col><Label>Revisado</Label><YN value={contactoRevisado} onChange={setContactoRevisado} /></Col>
            <Col><Label>Revisión SMA</Label><OkNec value={contactoRevisionSMA} onChange={setContactoRevisionSMA} /></Col>
          </Row2>
        </Card>
        ...
        <Card>
          ...
          <Row2>
            <Col><Label>Izquierdo (antes)</Label><YN value={bppDesgIzqAntes} onChange={setBppDesgIzqAntes} /></Col>
            <Col><Label>Izquierdo (después)</Label><YN value={bppDesgIzqDesp} onChange={setBppDesgIzqDesp} /></Col>
          </Row2>
          <Row2>
            <Col><Label>Derecho (antes)</Label><YN value={bppDesgDerAntes} onChange={setBppDesgDerAntes} /></Col>
            <Col><Label>Derecho (después)</Label><YN value={bppDesgDerDesp} onChange={setBppDesgDerDesp} /></Col>
          </Row2>

          <Label style={{ marginTop: 10 }}>Óxido</Label>
          <Row2>
            <Col><Label>Izquierdo (antes)</Label><YN value={bppOxIzqAntes} onChange={setBppOxIzqAntes} /></Col>
            <Col><Label>Izquierdo (después)</Label><YN value={bppOxIzqDesp} onChange={setBppOxIzqDesp} /></Col>
          </Row2>
          <Row2>
            <Col><Label>Derecho (antes)</Label><YN value={bppOxDerAntes} onChange={setBppOxDerAntes} /></Col>
            <Col><Label>Derecho (después)</Label><YN value={bppOxDerDesp} onChange={setBppOxDerDesp} /></Col>
          </Row2>

          <Label style={{ marginTop: 10 }}>Lubricación</Label>
          <Row2>
            <Col><Label>Izquierdo (antes)</Label><BM value={bppLubIzqAntes} onChange={setBppLubIzqAntes} /></Col>
            <Col><Label>Izquierdo (después)</Label><BM value={bppLubIzqDesp} onChange={setBppLubIzqDesp} /></Col>
          </Row2>
          <Row2>
            <Col><Label>Derecho (antes)</Label><BM value={bppLubDerAntes} onChange={setBppLubDerAntes} /></Col>
            <Col><Label>Derecho (después)</Label><BM value={bppLubDerDesp} onChange={setBppLubDerDesp} /></Col>
          </Row2>

          <Row2>
            <Col><Label>Revisado</Label><YN value={bppRevisado} onChange={setBppRevisado} /></Col>
            <Col><Label>Revisión SMA</Label><OkNec value={bppRevisionSMA} onChange={setBppRevisionSMA} /></Col>
          </Row2>
        </Card>
        ...
        <Card>
          <Row2>
            <Col><Label>Desgaste ¿tiene? (antes)</Label><YN value={tamborDesgTieneAntes} onChange={setTamborDesgTieneAntes} /></Col>
            <Col><Label>Desgaste ¿tiene? (después)</Label><YN value={tamborDesgTieneDesp} onChange={setTamborDesgTieneDesp} /></Col>
          </Row2>
          <Row2>
            <Col><Label>Aceite ¿tiene? (antes)</Label><YN value={tamborAceiteTieneAntes} onChange={setTamborAceiteTieneAntes} /></Col>
            <Col><Label>Aceite ¿tiene? (después)</Label><YN value={tamborAceiteTieneDesp} onChange={setTamborAceiteTieneDesp} /></Col>
          </Row2>
          <Row2>
            <Col><Label>Óxido ¿tiene? (antes)</Label><YN value={tamborOxidoTieneAntes} onChange={setTamborOxidoTieneAntes} /></Col>
            <Col><Label>Óxido ¿tiene? (después)</Label><YN value={tamborOxidoTieneDesp} onChange={setTamborOxidoTieneDesp} /></Col>
          </Row2>
          <Row2>
            <Col><Label>Revisado</Label><YN value={tamborRevisado} onChange={setTamborRevisado} /></Col>
            <Col><Label>Revisión SMA</Label><OkNec value={tamborRevisionSMA} onChange={setTamborRevisionSMA} /></Col>
          </Row2>
        </Card>
        ...
        <Card>
          <Row2>
            <Col><Label>¿Presenta desgaste/fisuras/aceite? Izq. (antes)</Label><YN value={balataIzqAntes} onChange={setBalataIzqAntes} /></Col>
            <Col><Label>Izq. (después)</Label><YN value={balataIzqDesp} onChange={setBalataIzqDesp} /></Col>
          </Row2>
          <Row2>
            <Col><Label>¿Presenta desgaste/fisuras/aceite? Der. (antes)</Label><YN value={balataDerAntes} onChange={setBalataDerAntes} /></Col>
            <Col><Label>Der. (después)</Label><YN value={balataDerDesp} onChange={setBalataDerDesp} /></Col>
          </Row2>
          <Row2>
            <Col><Label>Revisado</Label><YN value={balataRevisado} onChange={setBalataRevisado} /></Col>
            <Col><Label>Revisión SMA</Label><OkNec value={balataRevisionSMA} onChange={setBalataRevisionSMA} /></Col>
          </Row2>
        </Card>
        ...
        <Card>
          <Row2>
            <Col><Label>Prueba de funcionamiento (antes)</Label><BM value={operPruebaAntes} onChange={setOperPruebaAntes} /></Col>
            <Col><Label>Prueba de funcionamiento (después)</Label><BM value={operPruebaDesp} onChange={setOperPruebaDesp} /></Col>
          </Row2>
          <Row2>
            <Col><Label>Revisado</Label><YN value={operRevisado} onChange={setOperRevisado} /></Col>
            <Col><Label>Revisión SMA</Label><OkNec value={operRevisionSMA} onChange={setOperRevisionSMA} /></Col>
          </Row2>
        </Card>
...
function yn(v) { return v == null ? null : (v === 'Si' ? true : (v === 'No' ? false : null)); }
function bm(v) { return v == null ? null : (v === 'Bien' ? 'bien' : (v === 'Mal' ? 'mal' : null)); }
function oknec(v) { return v == null ? null : (v === 'Ok' ? 'ok' : (v === 'Necesita Seguimiento' ? 'seguimiento' : null)); }
