...
// ✅ IMPORTANTE: leer con EncodingType.Base64 y usar asset.localUri
export async function assetToDataUri(moduleAsset) {
  try {
    const asset = Asset.fromModule(moduleAsset);
    await asset.downloadAsync(); // 👈 necesario

    const localUri = asset.localUri; // 👈 NO uses asset.uri aquí
    if (!localUri) {
      console.log('[assetToDataUri] localUri null:', asset);
      return '';
    }
...
export async function buildPdfHtmlElevadores(payload, opts) {
  ...
  const SUBCONJUNTOS = [
    { titulo: '1.- CUARTO DE MAQUINA', items: ['1.1 PANEL DE CONTROL','1.2 FRENO','1.3 MÁQUINA DE TRACCIÓN/MOTOR/TACOGEN.','1.4 POLEA TRACCIÓN/DEFLECTORA','1.5 GOBERNADOR'] },
...
<body>
  ...
  <table>
    ...
    <tr>
      ...
      <!-- ✅ AQUÍ está lo compactado: SIN LÍNEAS, SIN ESPACIO EXTRA -->
